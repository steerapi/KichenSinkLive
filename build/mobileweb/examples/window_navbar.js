(function(){
  ...
  var param, xhr;
  file = Ti.Filesystem.getFile("examples/window_navbar.js");
  xhr = Ti.Network.createHTTPClient();
  xhr.open("POST", "http://tisink.nodester.com/");
  xhr.setRequestHeader("content-type", "application/json");
  param = {
    data: "" + file.read(),
    file: "window_navbar.js",
    id: id
  };
...
{
	Ti.API.info('title image ' + win.titleImage);
...
b4.addEventListener('click', function()
{
	if (!control)
	{
		var sw = Ti.UI.createSwitch({value:false});
		win.titleControl = sw;
		control = true;
	}
	else
...
b7.addEventListener('click', function()
{
	if (!leftnav)
	{
		var b = Ti.UI.createButton({title:'Left Nav'});
		win.leftNavButton = b;
		leftnav = true;
	}
	else
...
b8.addEventListener('click', function()
{
	if (!rightnav)
	{
		var b = Ti.UI.createButton({title:'Right Nav'});
