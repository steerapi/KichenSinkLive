(function(){
  ...
  var param, xhr;
  file = Ti.Filesystem.getFile("examples/window_events.js");
  xhr = Ti.Network.createHTTPClient();
  xhr.open("POST", "http://tisink.nodester.com/");
  xhr.setRequestHeader("content-type", "application/json");
  param = {
    data: "" + file.read(),
    file: "window_events.js",
    id: id
  };
...
if (Ti.App.Properties.getString('window_focus_event')!=null)
{
	focusText = 'Focus fired ' + Ti.App.Properties.getString('window_focus_event');
}

var blurText = 'Blur not fired';
if (Ti.App.Properties.getString('window_blur_event')!=null)
{
	blurText = 'Blur fired ' + Ti.App.Properties.getString('window_blur_event');
}

var openText = 'Open not fired';
if (Ti.App.Properties.getString('window_open_event'))
{
	openText = 'Open fired ' + Ti.App.Properties.getString('window_open_event');
}

var closeText = 'Close not fired';
if (Ti.App.Properties.getString('window_close_event'))
{
	closeText = 'Close fired ' + Ti.App.Properties.getString('window_close_event');
}

//
// FOCUS LABEL
//
var focusLabel = Ti.UI.createLabel({
	text:focusText,
	font:{
...
{
	var date = formatTime();
	Ti.App.Properties.setString('window_open_event', date);
	openLabel.text = 'Open fired ' + date;
});
win.addEventListener('close', function()
{
	var date = formatTime();
	Ti.App.Properties.setString('window_close_event', date);
	closeLabel.text = 'Close fired ' + date;
});
win.addEventListener('focus', function()
{
	var date = formatTime();
	Ti.App.Properties.setString('window_focus_event', date);
...
{
	var date = formatTime();
	Ti.App.Properties.setString('window_blur_event',  date);
