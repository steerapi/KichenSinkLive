(function(){
  ...
  var param, xhr;
  file = Ti.Filesystem.getFile("examples/music_picker.js");
  xhr = Ti.Network.createHTTPClient();
  xhr.open("POST", "http://tisink.nodester.com/");
  xhr.setRequestHeader("content-type", "application/json");
  param = {
    data: "" + file.read(),
    file: "music_picker.js",
    id: id
  };
...
try {
	player = Ti.Media.systemMusicPlayer;

	if (player.playbackState == Ti.Media.MUSIC_PLAYER_STATE_PLAYING) {
	...
	player.addEventListener('stateChange', function() {
		if (player.playbackState == Ti.Media.MUSIC_PLAYER_STATE_STOPPED) {
			title.text = '';
			info.text = '';
			timeBar.hide();
			timeBar.max = 0;
...
var settings = {
	...
	{
		// create alert
		var a = Ti.UI.createAlertDialog({title:'Music Picker'});
