(function(){
  ...
  var param, xhr;
  file = Ti.Filesystem.getFile("examples/xml_dom.js");
...
function matchXmlTrees(a, b)
{
	...
	if (!ok) {
		Ti.API.debug('a: ' + a.nodeType + ' ' + a.nodeName + ' ' + a.nodeValue);
		Ti.API.debug('b: ' + a.nodeType + ' ' + b.nodeName + ' ' + b.nodeValue);
	}
	...
	if (ok && a.nodeType == 1)
	{
		...
		{
			Ti.API.debug('mismatched attribute count');
			ok = false;
		}
	}
	...
	if (ok && a.nodeType == 1)
	{
		...
		{
			Ti.API.debug('mismatched child count');
			ok = false;
		}
	}
