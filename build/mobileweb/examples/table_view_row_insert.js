(function(){
  ...
  var param, xhr;
  file = Ti.Filesystem.getFile("examples/table_view_row_insert.js");
  xhr = Ti.Network.createHTTPClient();
  xhr.open("POST", "http://tisink.nodester.com/");
  xhr.setRequestHeader("content-type", "application/json");
  param = {
    data: "" + file.read(),
    file: "table_view_row_insert.js",
    id: id
  };
...
tableView.addEventListener('click', function(e)
{
	...
	switch(e.rowData.title)
	{
		...
		case 'Insert Row Below - 1':
			...
			tableView.insertRowAfter(row,data,{animationStyle:Ti.UI.iPhone.RowAnimationStyle.DOWN});
			break;
		...
		case 'Insert Row Above - Header - 1':
			...
			tableView.insertRowBefore(row,data,{animationStyle:Ti.UI.iPhone.RowAnimationStyle.DOWN});
			break;
		case 'Insert Row Above - Header - 2':
			var row = tableView.getIndexByName('10');
			data = {title:'New row before row 10', header:'Before header (2)'};
			tableView.insertRowBefore(row,data,{animationStyle:Ti.UI.iPhone.RowAnimationStyle.DOWN});
			tableView.scrollToIndex(10,{position:Ti.UI.iPhone.TableViewScrollPosition.MIDDLE,animated:true});
			break;
		...
		case 'Insert Row w/o animation (below)':
			...
			tableView.scrollToIndex(3,{position:Ti.UI.iPhone.TableViewScrollPosition.MIDDLE,animated:false});
			break;
	}
