(function(){
  ...
  var param, xhr;
  file = Ti.Filesystem.getFile("examples/filesystem.js");
...
// path variables
Ti.API.info('Resources Directory :' + Ti.Filesystem.resourcesDirectory);
Ti.API.info('Temp Directory :' + Ti.Filesystem.tempDirectory);
Ti.API.info('Application Directory :' + Ti.Filesystem.applicationDirectory);
Ti.API.info('Application Data Directory :' + Ti.Filesystem.applicationDataDirectory);
Ti.API.info('Application Support Directory :' + Ti.Filesystem.applicationSupportDirectory);

Ti.API.info('External Storage Available :' + Ti.Filesystem.isExternalStoragePresent());
Ti.API.info('Separator :' + Ti.Filesystem.separator);
Ti.API.info('Line Ending :' + Ti.Filesystem.lineEnding);


var f = Ti.Filesystem.getFile(Ti.Filesystem.resourcesDirectory, 'text.txt');
Ti.API.info('file = ' + f);
var contents = f.read();
...
var newFile = Ti.Filesystem.getFile(newDir.nativePath,'newfile.txt');
...
var renamedFile = Ti.Filesystem.getFile(newDir.nativePath, 'b.txt');
...
var l = Ti.UI.createLabel({text:'Check Log for details', width:300, height:'auto', textAlign:'center'});
win.add(l);

// test to make sure we can still access compiled JS files
var jsfile = Ti.Filesystem.getFile(Ti.Filesystem.resourcesDirectory,'app.js');
