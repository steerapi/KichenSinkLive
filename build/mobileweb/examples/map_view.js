(function(){
  ...
  var param, xhr;
  file = Ti.Filesystem.getFile("examples/map_view.js");
...
if (!isAndroid) {
	atlantaParams.pincolor = Ti.Map.ANNOTATION_PURPLE;
} else {
	atlantaParams.pinImage = "../images/map-pin.png";
}
var atlanta = Ti.Map.createAnnotation(atlantaParams);
...
var wireClickHandlers = function() {
	...
	sat.addEventListener('click',function() {
		...
		mapview.setMapType(Ti.Map.SATELLITE_TYPE);
	});
	...
	hyb.addEventListener('click',function() {
		...
		mapview.setMapType(Ti.Map.HYBRID_TYPE);
	});
...
if (!isAndroid) {
	...
	sat = Ti.UI.createButton({
		title:'Sat',
		style:Ti.UI.iPhone.SystemButtonStyle.BORDERED
	});
	// button to change map type to STD
	std = Ti.UI.createButton({
		title:'Std',
		style:Ti.UI.iPhone.SystemButtonStyle.BORDERED
	});
	// button to change map type to HYBRID
	hyb = Ti.UI.createButton({
		title:'Hyb',
		style:Ti.UI.iPhone.SystemButtonStyle.BORDERED
	});
	// button to zoom-in
	zoomin = Ti.UI.createButton({
		title:'+',
		style:Ti.UI.iPhone.SystemButtonStyle.BORDERED
	});
	// button to zoom-out
	zoomout = Ti.UI.createButton({
		title:'-',
		style:Ti.UI.iPhone.SystemButtonStyle.BORDERED
	});
...
mapview.addEventListener('click',function(evt)
{
	...
	if (myid == 3 && evt.clicksource == 'rightButton')
	{
		...
		evt.annotation.rightView = Ti.UI.createView({width:20,height:20,backgroundColor:'red'});
		evt.annotation.leftView = Ti.UI.createView({width:20,height:20,backgroundColor:'#336699'});
		evt.annotation.title = "Atlanta?";
		evt.annotation.pincolor = Ti.Map.ANNOTATION_GREEN;
